...
class Menu : AnyCommand {
    ...
    private val menu = """
        ...
        |8：coin，虚拟币交易市场，每天可签到一次获取金币
        |9：niu、牛子，击剑系统，使用help获取玩法说明
        |10：img、image，使用OpenaAI接口生成图
        |11：ph、pornhub，生成pornhub样式的图标
        |12：红白、5000、choyen，生成choyen5000样式图片
        |13：ygo、游戏王，生成游戏王卡片，可以选择类型和属性，指定群员，自定义名称和描述
        |    示例 .ygo 融合 灵摆 风 @群员 名称 描述 (顺序随意，可以用拼音缩写或者英文)
        |14：waifu、老婆，与随机群员结婚，可以@指定对象，可以自定义图片与角色名
        |    可以使用原神|崩坏三|碧蓝航线|碧蓝档案|明日方舟|公主连结角色名
        |15：news、60s，每天60秒读懂世界新闻
        |16：mute、禁言，随机禁言0-600秒
        |17：moyu、摸鱼，今日摸鱼人日历
        |18：tts、语音，微软文本转语音功能
        |19：kfc、肯德基，疯狂星期四文案
        |20：舔狗日记、舔狗，随机舔狗日记
        |21：js，执行js程序
        |22：recall、撤回，撤回机器人1-10条消息
        |23：error，查看最近一条机器人报错信息
        |24：state、状态，查看机器人运行状态
        |二、非指令功能
        |1：st、搜图，使用关键字回复一张图片，或者与自己的图片一起发送
        |2：sf、搜番，使用方法同上
        |3：喜报(xb)、悲报(bb)、结婚、离婚、毒瘾发作、一直、anti、work、china、陪睡券、
        | ： 恐龙、不要靠近、gun、共进晚餐、cheems、急急国王、loading、老婆、没有响应、
        | ： 小画家、prpr、砸屏、精神支柱、想什么、胡桃平板、翻转(flip) ，生成表情包
        |4：B站、V2EX、Github链接解析
        |5：合成两个emoji
        |6：自动回复，每40秒只能触发1次
        |7：二刺螈词库，@机器人常见词触发
