...
class Search : AnyCommand {
    ...
    override suspend fun execute(
        ...
        val image = message[Image.Key] ?: return PlainText("请发送一张图片").toMessageChain()

        val response1 = HttpUtil.getStringByProxy("$url$key&url=${image.queryUrl()}")
        if (response1?.statusCode() != 200) return PlainText("网络错误").toMessageChain()
        ...
        if (response2?.statusCode() != 200) return PlainText("网络错误").toMessageChain()
